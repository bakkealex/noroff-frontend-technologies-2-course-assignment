...
export default function AddStudioPage() {
  ...
  const handleAdd = async (data: {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to add studio");
      router.push("/admin/studios");
    } catch {
      setError("Failed to add studio."); // Redirect to studios list
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div>
      <h2 className="text-2xl font-semibold mb-6">Add new studio</h2>
      <div className="rounded bg-white dark:bg-gray-800 p-6 shadow max-w-lg">
        <AdminMiscForm label="studio" initialValues={{ name: "" }} onSubmit={handleAdd} submitting={submitting} error={error} />
        <BackButton />
      </div>
    </div>
  );
